...
import { PainPointIllustration } from './PainPointIllustration';
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
          {painPoints.items.map((item) => (
            ...
            >
              {/* Individual editorial illustration */}
              <div className="w-full h-40 rounded-xl overflow-hidden mb-4 bg-gradient-to-br from-white via-brand-50/55 to-pinkAccent relative">
                <PainPointIllustration id={item.id} />

                <div className="absolute top-3 left-3 w-7 h-7 rounded-full bg-white/90 backdrop-blur-md text-brand-600 font-extrabold text-xs flex items-center justify-center shadow-sm border border-white">
                  {item.id}
                </div>
              </div>
